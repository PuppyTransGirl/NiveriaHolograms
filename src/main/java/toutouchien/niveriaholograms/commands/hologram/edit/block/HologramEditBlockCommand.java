                ...
                .then(Commands.argument("block", ArgumentTypes.blockState())
                        .executes(ctx -> {
                            ...
                            if (!(hologram.configuration() instanceof BlockHologramConfiguration)) {
                                LANG.sendMessage(sender, "command.hologram.edit.only_block");
                                return Command.SINGLE_SUCCESS;
                            }

                            if (blockState.getType().isAir()) {
                                LANG.sendMessage(sender, "command.hologram.edit.block.no_air");
                                return Command.SINGLE_SUCCESS;
                            }
                            ...
                            LANG.sendMessage(sender, "command.hologram.edit.block.edited",
                                    Lang.unparsedPlaceholder("hologram_name", hologramName),
                                    Lang.unparsedPlaceholder("hologram_block_block", blockState.getType().translationKey())
                            );
                            return Command.SINGLE_SUCCESS;
                        })
                ).build();
